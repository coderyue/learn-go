...
import (
	//_ "l_package/pk1" //如果指向执行包下的init()函数，那么前面加个_
	"database/sql"
	"fmt"
...
func main() {
	...
	5. main包， main()函数所在的包， 其他的包不能使用(不能起相同的名字)
	...
	*/
	...
	7._操作，其实是引入该包， 而不直接使用包里的函数，仅仅是调用了该包里的init()
	 */
	...
	// database/sql
	db, err := sql.Open("mysql", "root:root@tcp(127.0.0.1:3308)/test?charset=utf8")
	if err != nil {
